...
export class ValidateNotificationService {
  ...
  validateNotifiedPerson(notifiedPerson: NotifiedPerson): boolean {
    return (
      !!notifiedPerson &&
      this.validateNotifiedPersonInfo(notifiedPerson) &&
      (this.validateInternationalAddress(notifiedPerson.primaryAddress) ||
        this.validateInternationalAddress(notifiedPerson.ordinaryAddress) ||
        this.validateInternationalAddress(notifiedPerson.currentAddress)) &&
      this.validateContactPointInfoNotifiedPerson(notifiedPerson.contacts)
    );
  }

  validateDnNotifiedPerson(notifiedPerson: NotifiedPerson): boolean {
    return (
      !!notifiedPerson &&
      this.validateNotifiedPersonInfo(notifiedPerson) &&
      this.validateInternationalAddress(notifiedPerson.primaryAddress) &&
      this.validateContacts(notifiedPerson.contacts)
    );
  }

  validateNotifiedPersonInfo(notifiedPerson: NotifiedPerson): boolean {
    return (
      !!notifiedPerson?.info &&
      this.validateName(notifiedPerson.info.firstname, true) &&
      this.validateName(notifiedPerson.info.lastname, true) &&
      !!notifiedPerson.info.gender &&
      this.isDateInputValid(notifiedPerson.info.birthDate, false)
    );
  }
  ...
  isDateInputValid(date: string | undefined, required: boolean): boolean {
    return required ? !!date && !validateDateInput(date) : !date || !validateDateInput(date);
  }

  isPartialDateInputValid(date: string, required: boolean): boolean {
    return required ? !!date && !validatePartialDateInput(date) : !date || !validatePartialDateInput(date);
  }

  validateDuration(startDate: string, endDate: string): boolean {
    return !!startDate && !!endDate ? isEndDateLaterThanStartDate(startDate, endDate) : true;
  }
  ...
  validateFacilityInfo(facilityInfo: FacilityInfo, requiredBsNr: boolean) {
    // if (!facilityInfo) return true; // kann/sollte entfallen sobald wir Typen wirklich ernst nehmen
